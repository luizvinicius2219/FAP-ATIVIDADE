# 📊 Atividade Prática | FAP 2026 🚀
### Curso de Formação Acelerada em Programação • Trilha de Análise de Dados

<p align="center">
  <img src="https://img.shields.io/badge/Curso-An%C3%A1lise%20de%20Dados-blue?style=for-the-badge&logo=data-science&logoColor=white" alt="Trilha">
  <img src="https://img.shields.io/badge/Ano-2026-green?style=for-the-badge" alt="Ano">
  <img src="https://img.shields.io/badge/Status-Em%20Desenvolvimento-yellow?style=for-the-badge" alt="Status">
</p>

---

## 📌 Sobre o Projeto
Este repositório foi criado para centralizar e apresentar as atividades desenvolvidas durante o módulo prático do curso **FAP 2026 (Formação Acelerada em Programação)**. O objetivo principal é aplicar técnicas de manipulação, limpeza, exploração e visualização de dados para extrair *insights* valiosos a partir de bases de dados reais.

---

## 🛠️ Tecnologias e Ferramentas Utilizadas
O ecossistema de ferramentas utilizado para a resolução desta atividade inclui:

* 🐍 **Python** — Linguagem base para análise.
* 🐼 **Pandas & NumPy** — Manipulação e estruturação dos dados.
* 📊 **Matplotlib & Seaborn** — Construção de gráficos e visualizações.
* 📓 **Jupyter Notebook / VS Code** — Ambiente de desenvolvimento.

---

## 👨‍💻 Autor

* **Luiz Vinicius** — [@luizvinicius2219](https://github.com/luizvinicius2219)
* **E-mail:** luizvinicius0912@gmail.com

---
<p align="center">Desenvolvido com 💙 para a jornada de dados no FAP 2026.</p>
